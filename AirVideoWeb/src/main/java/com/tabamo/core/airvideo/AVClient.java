...
public class AVClient {
	...
	public AVClient(String server, int port, String password) {
	...
	public ArrayList <AVResource> ls(AVFolder dir) {
	...
	public AVFolder cd(AVFolder dir) {
		pwd = dir.location.substring(1);
		AVFolder f = new AVFolder(this, dir.name, pwd);
		return f;
	}
	
	public URL getUrl(AVVideo video, boolean live) throws MalformedURLException {
		AVMap packet;
		URL contentURL;
		AVMap result;
	...
	public AVMap request(String service, String method, Object params) {
...
//			avrequest = AVMap.parse(bis);
			...
			int numread;
			while ((numread = request.getInputStream().read(buffer)) != -1) {
				responseBuffer.write(buffer, 0, numread);
			}
			
			ByteArrayInputStream responseStream = new ByteArrayInputStream(
					responseBuffer.toByteArray());
			DataInputStream dataInputStream = new DataInputStream(responseStream);
			
			avrequest = AVMap.parse(dataInputStream);
		} 
		catch (Exception e) {
			e.printStackTrace();
			avrequest = null;
